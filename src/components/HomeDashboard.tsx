    ...
    <div className="space-y-8 pb-12">
      ...
      <section className="relative rounded-3xl overflow-hidden bg-slate-900 text-white shadow-xl min-h-[420px] flex items-center">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-6 py-12 sm:py-16 text-center space-y-6">
          ...
          {/* 17. PERSONALIZED HOME SCREEN OR DEFAULT SEARCH FORM */}
          {currentUser?.travelProfile ? (
            <div className="bg-white/95 backdrop-blur-md p-5 sm:p-6 rounded-3xl shadow-2xl border border-white/40 text-slate-800 max-w-3xl mx-auto space-y-4 text-left animate-in fade-in duration-300">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-3">
                <div>
                  ...
                  <h3 className="text-xs sm:text-sm font-bold text-blue-700">
                    Ready for your next trip?
                  </h3>
                </div>
                <button
                  id="btn-home-plan-new-trip"
                  type="button"
                  onClick={onPlanNewTrip}
                  className="px-3.5 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs rounded-xl transition-colors flex items-center gap-1.5 cursor-pointer self-start sm:self-auto"
                >
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 rounded-2xl bg-gradient-to-r from-blue-50/80 via-indigo-50/60 to-slate-50 border border-blue-100">
                <div className="space-y-1">
                  ...
                  <div className="text-xs text-slate-600 font-semibold flex flex-wrap items-center gap-2">
                    <span>{currentUser.travelProfile.availableDays} Days</span>
                    <span>•</span>
                    <span>{currentUser.travelProfile.travelersCount} Travelers ({currentUser.travelProfile.travelersType})</span>
                    <span>•</span>
                    <span className="font-bold text-emerald-700">
                      ₹{currentUser.travelProfile.budgetAmount.toLocaleString('en-IN')}
                    </span>
                    <span>•</span>
                    <span className="bg-white px-2 py-0.5 rounded-md border border-slate-200 text-[11px] font-bold text-slate-700">
                      {currentUser.travelProfile.travelStyle} Style
                    </span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <button
                    id="btn-home-continue-planning"
                    type="button"
                    onClick={() => {
                      onStartPlanTrip({
                        destination: currentUser.travelProfile?.destination || 'Mysuru',
                        date: currentUser.travelProfile?.startDate || '2026-09-12',
                        travelers: currentUser.travelProfile?.travelersCount || 2,
                      });
                    }}
                    className="py-3 px-6 bg-blue-600 hover:bg-blue-700 text-white font-extrabold text-xs rounded-xl shadow-md transition-all flex items-center gap-1.5 cursor-pointer whitespace-nowrap group"
                  >
              ...
              {/* Budget warning & auto optimization note if exceeds */}
              {currentUser.travelProfile.budgetBreakdown?.exceedsBudget && (
                <div className="p-3 bg-amber-50 border border-amber-200 rounded-xl space-y-1">
                  ...
                  <ul className="text-[11px] text-amber-800 list-disc list-inside">
                    {currentUser.travelProfile.budgetBreakdown.optimizationNotes?.map((note, i) => (
                      <li key={i}>{note}</li>
                    ))}
                  </ul>
                </div>
      ...
      <section className="bg-gradient-to-br from-amber-500/10 via-blue-500/5 to-emerald-500/10 border-2 border-amber-500/30 rounded-2xl p-5 sm:p-6 shadow-sm">
        ...
        {/* Personalized Recommendation Card if user profile exists */}
        {currentUser?.travelProfile && (
          <div className="mt-4 p-4 rounded-xl bg-gradient-to-r from-emerald-50 via-teal-50 to-blue-50 border border-emerald-200 shadow-2xs text-left">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-2">
              <div className="flex items-center gap-2">
                <span className="px-2 py-0.5 bg-emerald-600 text-white text-[10px] font-black uppercase rounded-md tracking-wider">
                  Personalized For {currentUser.name}
                </span>
                <span className="text-xs font-bold text-slate-700">
                  {currentUser.travelProfile.startingLocation} → {currentUser.travelProfile.destination}
                </span>
              </div>
              <span className="text-xs font-black text-emerald-800 bg-white/80 px-2.5 py-1 rounded-full border border-emerald-200 self-start sm:self-auto">
                Strict Budget Cap: ₹{currentUser.travelProfile.budgetAmount.toLocaleString('en-IN')}
              </span>
            </div>

            <p className="text-xs text-slate-700 leading-relaxed font-medium">
              🎯 Engine prioritized: <strong>{currentUser.travelProfile.interests.join(', ')}</strong> • Preferred transport: <strong>{currentUser.travelProfile.transportPreference.join(' + ')}</strong> • Stay: <strong>{currentUser.travelProfile.hotelPreference}</strong> (approx ₹{Math.round(currentUser.travelProfile.budgetAmount * 0.35 / Math.max(1, currentUser.travelProfile.availableDays - 1)).toLocaleString('en-IN')}/night) • Total estimated cost guaranteed within ₹{currentUser.travelProfile.budgetAmount.toLocaleString('en-IN')}.
            </p>

            <div className="mt-3 flex flex-wrap items-center justify-between gap-2 border-t border-emerald-100 pt-2.5">
              <div className="flex flex-wrap items-center gap-2 text-[11px] text-emerald-800 font-bold">
                <span className="inline-flex items-center gap-1">
                  <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" />
                  Short-Distance Route (Min Transit)
                </span>
                <span>•</span>
                <span className="inline-flex items-center gap-1">
                  <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" />
                  Affordable Verified Stays
                </span>
              </div>
              <button
                id="btn-rec-open-personalized-itinerary"
                onClick={() => {
                  onStartPlanTrip({
                    destination: currentUser.travelProfile?.destination || 'Mysuru',
                    date: currentUser.travelProfile?.startDate || '2026-09-12',
              ...
              >
                <span>Generate {currentUser.travelProfile.availableDays}-Day Custom Itinerary</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
          ...
          <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-2xs hover:border-amber-400 transition-colors text-left">
          ...
          <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-2xs hover:border-blue-400 transition-colors text-left">
